...
with open ('credential.json', "r") as cred:                                         # cred = open('credential.json', 'r')
        credential = json.load(cred)                                                # credential = json.load(cred)
...
def insert_raw_data():
  ...
  with open ('D:/all about python/Project 3/data/data_covid.json', "r") as data:
    data = json.load(data)

  df = pd.DataFrame(data['data']['content'])

  df.columns = [x.lower() for x in df.columns.to_list()]
  df.to_sql(name='handra_raw_covid', con=engine, if_exists="replace", index=False)
  engine.dispose()
...
def insert_raw_to_warehouse(schema):
    mysql_auth = MySQL(credential['mysql_lake'])
    engine, engine_conn = mysql_auth.connect()
    data = pd.read_sql(sql='handra_raw_covid', con=engine)
    engine.dispose()
